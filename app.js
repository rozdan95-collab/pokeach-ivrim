(() => {
  ...
  // Optional: per-page link(s) to the original manuscript image in Google Drive.
  // You can set a single string OR an object with {right:{label,url}, left:{label,url}}
  const ORIG_PAGE_LINKS = {
    9: {
      right: {
        label: "דף א׳ ע״ב (ימין)",
        url: "https://drive.google.com/file/d/142isFumVOfZTfbxn3T_OPUwwaR14sRf_/view?usp=drive_link"
      },
      left: {
        label: "דף ב׳ ע״א (שמאל)",
        url: "https://drive.google.com/file/d/142isFumVOfZTfbxn3T_OPUwwaR14sRf_/view?usp=drive_link"
  ...
  function renderSidebar() {
    ...
    if (notes.length === 0) {
      notesRoot.innerHTML =
        "<div style='color:#666'>אין עדיין הערות. במצב עריכה: לחץ על מילה כדי להוסיף פירוש.</div>";
      return;
    }

    notes
      .slice()
      .sort((a, b) => (b.page - a.page) || (b.word_index - a.word_index))
      .forEach((n) => {
        ...
        d.innerHTML = `
          ...
          <div class="meta">
            ...
            <span style="display:flex;gap:6px;flex-wrap:wrap">
              ...
              ${
                EDIT_MODE
                  ? `<button data-act="edit" data-id="${n.id}">ערוך</button><button data-act="del" data-id="${n.id}">מחק</button>`
                  : ``
              }
            </span>
          </div>
  ...
  // --- orig link (supports 1 link or right/left links) ---
  function updateOrigLink(page) {
    ...
    if (entry.right && entry.right.url) {
      parts.push(
        `<a href="${entry.right.url}" target="_blank" rel="noopener">${esc(entry.right.label || "צד ימין")}</a>`
      );
    }
    if (entry.left && entry.left.url) {
      parts.push(
        `<a href="${entry.left.url}" target="_blank" rel="noopener">${esc(entry.left.label || "צד שמאל")}</a>`
      );
    }
  ...
  notesRoot.addEventListener("click", (e) => {
    ...
    if (act === "goto") {
      renderPage(n.page).then(() => toggleSidebar(false));
      return;
    }
